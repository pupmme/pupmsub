...
install() {
    bash <(curl -Ls https://raw.githubusercontent.com/pupmme/pupmsub/script/install.sh)
    if [[ $? == 0 ]]; then
        if [[ $# == 0 ]]; then
            start
        else
...
update() {
    ...
    bash <(curl -Ls https://raw.githubusercontent.com/pupmme/pupmsub/script/install.sh) $version
    if [[ $? == 0 ]]; then
...
update_shell() {
    wget -O /usr/bin/V2bX -N --no-check-certificate https://raw.githubusercontent.com/pupmme/pupmsub/script/V2bX.sh
    if [[ $? != 0 ]]; then
        echo ""
